...
基于 2026-05-19 的 5 工具 × 15 测试用例 × 34 组合对比评测结论。评分方法：LLM-as-Judge (GLM-5.1)，4 维度 1-5 分。LLM-Judge 验证一致（avg deviation 0.57）。
...
2. **Context7 仅限编程文档**：不适合代码调试（OS 级运行时错误）、不适合国内技术（智谱 GLM 等）、不适合技术选型对比（只返回单库文档）。
...
- `results/` — 原始测试结果 JSON（34 个工具-TC 组合）
