...
[English](../README.md) | [工具选择指南](../搜索工具选择指南.md) | [迭代记录](../CHANGELOG.md)
...
`search_selector.py` 提供程序化的工具选择：

```python
from search_selector import recommend_tool, format_recommendation

rec = recommend_tool("Next.js middleware 配置")
print(format_recommendation(rec))
# → 首选: Context7（编程文档，零噪音）
# → 备选: WebSearch
```

支持 7 种意图类型（code/search/extract/research/finance/academic/security），自动语言检测（中/英），以及深度感知推荐（quick/deep）。
