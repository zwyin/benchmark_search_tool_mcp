...
def generate_report(tool_scores, test_cases):
    ...
    lines.append("")
    lines.append("### 发现3: Context7 是编程文档的绝对王者，但仅限于 API/SDK 文档")
    lines.append("- 直接返回官方仓库的代码片段，零噪音，Agent 可直接粘贴使用")
    lines.append("- TC03 Claude Code 插件开发：满分 5/5/5/5，精准命中 plugin.json/Agent SDK/hooks/MCP 配置")
    lines.append("- TC14 Dify v0.15 release notes：仅 2/1/3/2，只能查到 Workflow API 文档，无法获取版本更新内容")
    lines.append("- **边界明确**：适合查库/框架的 API 和配置示例，不适合版本追踪、changelog、release notes")
    lines.append("")
    ...
    lines.append("")
    lines.append("### 发现9: 学术搜索用 WebSearch，不要用 AnySearch 垂直")
    lines.append("- TC12 transformer attention survey：AnySearch 垂直学术搜索返回点云分析论文而非 attention 综述（3/3/4/3）")
    lines.append("- 同一查询 WebSearch 返回 arXiv/OpenReview/Semantic Scholar 精准匹配结果（5/5/5/5）")
    lines.append("- **建议**：学术搜索用 WebSearch，不要用 AnySearch 垂直学术")
    lines.append("")
    ...
    lines.append("- **AnySearch 垂直领域搜索**：已测试金融(AAPL)、学术(transformer survey)、安全(CVE-2024-9264)三个垂直领域，其余垂直领域（如法律/医疗等）未测试")
    lines.append("- **时效性衰减**：未测试同一查询在不同时间点的结果差异")
    lines.append("")
    lines.append("### 改进建议")
    lines.append("1. 在自动化框架中加入精确 token 计数和延迟计时")
    lines.append("2. 测试搜索结果的时效性衰减（同一查询在不同时间点的结果差异）")
    lines.append("3. 测试更多 AnySearch 垂直领域（法律/医疗等，当前已测金融/学术/安全三个）")
